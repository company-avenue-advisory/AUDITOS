...
class TestBuildSummaries(unittest.TestCase):
    ...
    def test_filings_summary_shape(self):
        ...
        self.assertTrue(summary["27AAAAA1111A1Z1"]["has_data"])
        self.assertEqual(summary["27AAAAA1111A1Z1"]["total_taxable"], 100.0)
        self.assertEqual(summary["27AAAAA1111A1Z1"]["flagged_unresolved"], ["MH1"])
...
class TestPeriodReviewLifecycle(unittest.TestCase):
    ...
    def test_get_review_detail_deserializes_json_fields(self):
        ...
        self.assertIn("27AAAAA1111A1Z1", detail["filings_summary"])
...
class TestGeneratePeriodReviewForTenant(unittest.TestCase):
    ...
    def setUp(self):
        ...
        Base.metadata.create_all(engine)
        self.db = sessionmaker(bind=engine)()
        self.db.add(Tenant(id="t1", name="Test Tenant", slug="test-tenant"))
        self.db.add(User(id="u1", email="reviewer@example.com", hashed_password="x", tenant_id="t1"))
        self.db.add(BatchJob(id="b1", tenant_id="t1", status=TaskStatus.COMPLETED))
        self.db.add(InvoiceTask(id="task1", batch_id="b1", file_name="INVOICE1.pdf", status=TaskStatus.COMPLETED))
        self.db.add(SalesLineItem(
            task_id="task1", voucher_date="15-06-2026", voucher_type="Sales",
            invoice_no="MH1", party_ledger_name="Test Bank Ltd", party_gstin="27AAAAA0000A1Z5",
        ...
        _build_client_sheet(self.sheet_path, [[
            1, "Invoice", "MH1", "15-06-2026", "Test Bank Ltd", "27AAAAA0000A1Z5",
            "B2B", "27", "Intrastate", 76.61, 0.0, 6.89, 6.89, 90.39,
        ]])

        # generate_period_review_for_tenant loads its registration map from
        # a gitignored data/registration_maps/{slug}.json (real tenant
        # config, never committed) - patched here so this test doesn't
        # depend on that file existing on whatever machine runs the suite.
        self.reg_map_patcher = patch(
            "services.gstr1_filing.load_registration_map",
            return_value={"MH": "27TESTBK0000T1Z1", "OMH": "27TESTBK0000T1Z1"},
        )
        self.reg_map_patcher.start()

    def tearDown(self):
        shutil.rmtree(self.tmp_dir, ignore_errors=True)
        self.reg_map_patcher.stop()
    ...
    def test_persisted_review_flags_totals_mismatch_from_uncategorized_credit_note(self):
        ...
        self.db.add(SalesLineItem(
            ...
            invoice_no="CR1", party_ledger_name="Test Bank Ltd", party_gstin="27AAAAA0000A1Z5",
        ...
        _build_client_sheet(sheet_with_cn, [
            [1, "Invoice", "MH1", "15-06-2026", "Test Bank Ltd", "27AAAAA0000A1Z5",
             "B2B", "27", "Intrastate", 76.61, 0.0, 6.89, 6.89, 90.39],
            [2, "Credit Note", "CR1", "20-06-2026", "Test Bank Ltd", "27AAAAA0000A1Z5",
